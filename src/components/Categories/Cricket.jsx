...
const Cricket = () => {
    ...
    return (
        <div className='container d-flex flex-wrap'>
            <Helmet>
                <title>Live Cricket Score | Scorecard | Live Commentary</title>
                ...
                <link rel="canonical" href="https://cricpoint.in/cricket" />
            </Helmet>
            {cat.map((po) => {
                return (
                    <div key={po.id} class="card m-4" style={{ width: "18rem" }}>
                        <img class="card-img-top" style={{ width: "auto", height: "200px" }} src={po.featured_media_src_url} alt="Card image cap" />
                        <div class="card-body">
                            <h5 class="" dangerouslySetInnerHTML={{ __html: po.title.rendered }}></h5>
                            <p class="card-text" dangerouslySetInnerHTML={{ __html: po.excerpt.rendered }}></p>
                            {/* <link to="/post/${po.id}">read more</link> */}
                            <Link to={`/post/${po.slug}`} class="btn btn-primary">Read More..</Link>
                        </div>
                    </div>
                )
            })}
